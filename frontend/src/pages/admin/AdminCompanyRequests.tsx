...
const AdminCompanyRequests = () => {
  ...
  const [selected, setSelectedMenu] = useState("Requests")
  const [comapnyDetail,setComapnyDetail]=useState<Company[]>()
...
const handleStatusChange = async (companyId: string, newStatus: string) => {
  ...
  useEffect(() => {
    const fetchCompany = async () => {
      try {
        ...
        setComapnyDetail(response.company)
        
      } catch (error) {
        console.log('error in fetching comapny')
      }
  ...
  { comapnyDetail&& comapnyDetail.length > 0 ? (
    comapnyDetail.map((company) => (
      <div key={company._id} className="grid grid-cols-7 items-center bg-white shadow-md p-4 rounded-lg my-2">
        ...
        <h1
          className={`text-center font-medium ${
            company.status === "Pending" ? "text-yellow-500" : "text-green-600"
          }`}
        >
          {company.status}
        </h1>
  ...
  <button
    className={`px-4 py-2 text-white rounded-md flex items-center ${
      company.status === "Pending" ? "bg-orange-600" :
      company.status === "Approved" ? "bg-green-600" : "bg-red-600"
    }`}
  ...
  {openDropdown === company._id && (
    <div className="absolute top-full mt-2 bg-white shadow-md rounded-md w-28 text-center z-1">
      <button
