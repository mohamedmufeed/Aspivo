...
import { PopulatedSavedJob } from "../../../types/userTypes";

export default interface IJobService{
    fetchJob(page:number,limit:number,searchWord?:string, category?:string):Promise<{job:IJob[],total:number,page:number,totalPages:number, message:string}>
